...
def append_sql_statements(module_id, video_id, course_id, sequence_no, video_title, video_description, video_url, duration):
...
def get_next_module_id():
    try:
        ...
        max_id = int(resp.json().get("max_module_id", 0))
        return str(max_id + 1)
    except Exception:
        return "1"
...
def get_next_video_id():
    try:
        ...
        resp.raise_for_status()
        max_id = int(resp.json().get("max_video_id", 0))
        return str(max_id + 1)
    except Exception:
        return "1"
...
if __name__ == "__main__":
    # Specify course_id manually here
    ...
    # Truncate the course_content_operations table before processing
    ...
    for video in tqdm(videos, desc="Processing videos", unit="video"):
        ...
        try:
            ...
            # Change prompts to request only one concise title/description
            video_title_raw = get_chat_response(
                f"Based on the following transcript, provide ONE concise and clear video title (no lists, no markdown, no quotes):\n{transcript}"
            )
            ...
            # Get next IDs from API
            module_id = get_next_module_id()
            video_id = get_next_video_id()

            # Append SQL statements to file
            append_sql_statements(module_id, video_id, course_id, sequence_no, video_title, video_description, video_url, duration)
